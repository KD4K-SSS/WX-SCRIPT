...
# -------------------------
# ATIS LETTER SYSTEM (A→Z PERFECT)
# -------------------------
...
# -------------------------
FORECAST_URL="https://api.weather.gov/points/33.68,-78.89/forecast"
FORECAST_JSON=$(curl -s "$FORECAST_URL")

NOW_EPOCH=$(date +%s)
FORECAST_PHRASE="No significant changes expected"

FORECAST_MATCH=$(echo "$FORECAST_JSON" | jq -r '
.properties.periods[] |
"\(.startTime)|\(.detailedForecast)"
' | while IFS="|" read -r START TEXT; do
  START_EPOCH=$(date -d "$START" +%s 2>/dev/null)
  DIFF=$(( (START_EPOCH - NOW_EPOCH) / 3600 ))
...
if [ -n "$FORECAST_MATCH" ]; then
  FORECAST_SHORT=$(echo "$FORECAST_MATCH" | cut -d'.' -f1)

  if echo "$FORECAST_MATCH" | grep -qiE "storm|thunder|rain|snow|fog|wind|gust|hail|tornado"; then
    FORECAST_PHRASE=$(echo "$FORECAST_SHORT" | sed -E 's/^(This|Tonight|Today)/Expect/i')
  else
    FORECAST_PHRASE="No significant changes expected"
  fi
fi
