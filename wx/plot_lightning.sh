...
MY_LAT=33.6211836
MY_LON=-78.9648714
INTERVAL=180 # 3 minutes execution cycle

while true; do
    echo "=========================================="
    echo "Starting Plotting Engine: $(date '+%Y-%m-%d %H:%M:%S')"
    echo "=========================================="
    ...
    do
        if [ ! -f "$FILE" ]; then
            echo "ERROR: Missing required source file: $FILE"
            MISSING_FILE=1
        fi
    done

    # If file assets are missing this pass, wait and attempt recovery next loop
    if [ "$MISSING_FILE" -ne 0 ]; then
        echo "Source assets missing. Skipping cycle..."
    else
    ...
    for ((sec=INTERVAL; sec>0; sec--)); do
        printf "\rNext map render in %02d:%02d   " $((sec/60)) $((sec%60))
        sleep 1
    done
    echo ""
done
